...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Função para preencher os campos do formulário com dados do banco
  const loadBuildData = async () => {
    try {
      // Simulação de fetch para obter dados do banco
      const response = await fetch("/getBuild", { method: "GET" });
      if (!response.ok) {
      ...
      // Preencher os campos de stats
      document.getElementById("finalLevel").value = stats.level || 0;
      document.getElementById("finalVigor").value = stats.vigor || 0;
      document.getElementById("finalAttunement").value = stats.attunement || 0;
      document.getElementById("finalEndurance").value = stats.endurence || 0;
      document.getElementById("finalVitality").value = stats.vitality || 0;
      document.getElementById("finalStrength").value = stats.strength || 0;
      document.getElementById("finalDexterity").value = stats.dexterity || 0;
      document.getElementById("finalIntelligence").value = stats.inteligence || 0;
      document.getElementById("finalFaith").value = stats.faith || 0;
      document.getElementById("finalLuck").value = stats.luck || 0;

      // Preencher os campos da build
      document.getElementById("buildName").value = build.name || "";
      document.getElementById("class").value = build.class || "";

      // Preencher os equipamentos
      equipment.forEach((equip) => {
        // Procurar pelo campo correspondente ao tipo de equipamento
        const selectId = {
          weapon: ["R1Select", "R2Select", "R3Select", "L1Select", "L2Select", "L3Select"],
          ring: ["Ring1Select", "Ring2Select", "Ring3Select", "Ring4Select"],
          helmet: ["helmetSelect"],
          chest: ["chestSelect"],
          hands: ["handsSelect"],
          legs: ["legsSelect"],
        ...
        // Encontrar o próximo campo vazio no grupo do tipo
        if (selectId) {
          const emptyField = selectId.find((id) => document.getElementById(id).value === "");
          if (emptyField) {
            document.getElementById(emptyField).value = equip.name;
          }
        }
      });
    } catch (error) {
